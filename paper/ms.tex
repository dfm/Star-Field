...
\right.
\end{eqnarray}
We will need the fully normalised version. Putting in two normalising constants:
\begin{eqnarray}
p(x | \theta) &=&
\left\{
\begin{array}{lcr}
0, & & x < x_0 \\
C_1x^{-\alpha_1 - 1}, & & x_0 \leq x \leq x_1 \\
C_2x^{-\alpha_2 - 1}, & & x > x_1
\end{array}
\right.
\end{eqnarray}
The normalising constants can be obtained by enforcing the following
constraints. The first constraint is that the total probability is unity:
\begin{eqnarray}
\int p(x|\theta) \, dx = 1
\end{eqnarray}
The second constraint is that the PDF should be continuous at $x=x_1$:
\begin{eqnarray}
C_1 x_1^{-\alpha_1 - 1} = C_2 x_1^{-\alpha_2 - 1}
\end{eqnarray}
