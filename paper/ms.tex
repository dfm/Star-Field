...
of each star is uncertain. The method is implemented using advanced
Markov Chain Monte Carlo (MCMC) techniques including Reversible Jump
and Nested Sampling.
This is in contrast with standard techniques which typically produce a single catalog,
potentially underestimating the uncertainties in any study of the stellar
population and discarding information about sources at or below the detection limit.
While standard approaches are often successful, they are more likely to fail
when the image contains many overlapping sources.
