...
to be inferred, ii) the objects that we expect to find are point sources such
...
circular
Gaussians (a similar mixture model is also used by DAOPHOT, \citet{1987PASP...99..191S}) with widths
$s_1$ and $s_2$:
\begin{eqnarray}
...
function. The resulting CLF from this run did underestimate the number of low stars.
However, the
...
which is a significant issue in practice. To build a more complete picture of
when our approach is necessary in practice, it will need to be tested against
a wider variety of alternative methods.

We note that there are many limitations to the model presented in this paper,
some of which will be important to relax when applying it to real data.
