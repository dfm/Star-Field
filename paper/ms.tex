...
to predict the data if we happened to know the true catalog. Thus, we need
a prescription for simulating images from a catalog:
\begin{eqnarray}
\theta = \left\{N, \beta, \left\{x_i, y_i\right\}_{i=1}^N, 
\left\{f_i\right\}_{i=1}^N\right\}
\end{eqnarray}
If we knew the true catalog, we could compute the image we would expect to see,
in the absence of noise. This mock image is given by:
\begin{eqnarray}
...
\right].
\end{eqnarray}
The observed image is assumed to be generated from the mock image (evaluated
at the center of each pixel) plus Gaussian noise:
\begin{eqnarray}
...
$N_{\rm max}$. The model for the luminosity function is a broken power-law.
