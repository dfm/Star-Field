...
with those obtained from the \sex~software and find that the latter is
unable to correctly infer the number of stars or the parameters of the luminosity
function.
\end{abstract}
...
incident intensity field.
Standard tools for generating catalogs from astronomical imaging include \sex~\citep{sextractor},
DAOPHOT \citep{1987PASP...99..191S}, and SDSS PHOTO \citep{photo}.
...
This Article represents an attempt at this ambitous goal.
...
tightly constrain the parameters. A Nested Sampling approach is required on
...
(just the positions of the stars are shown) sampled from the posterior
distribution. Features that are common to these nine samples are plausible, and
features that differ are uncertain.

\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{Figures/catalogs.eps}
\end{center}
...
with the blurred (but still noise-free) version and the residuals.
...
uncertainty. We now compare this approach with the output from the standard
tool \sex. To achieve this, we executed \sex~on the two test cases, for various
values of the significance level cutoff. This results in a set of catalogs
for each image, with more conservative cutoffs resulting in less stars detected
as compared to more agressive cutoffs. The results for the number of stars in
...
possible values of the cutoff parameter.
...
cutoff and fitted a broken power-law model to the list of fluxes in the catalog.
The results of this inference are shown in Figure~\ref{fig:sex_inference}.
